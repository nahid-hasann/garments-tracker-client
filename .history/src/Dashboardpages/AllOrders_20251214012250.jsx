...
const AllOrders = () => {
    // ১. ফিল্টার এবং সার্চের জন্য স্টেট ডিক্লেয়ার
    ...
    const { data: orders = [], isLoading } = useQuery({
        queryKey: ["all-orders"],
        queryFn: async () => {
            const res = await axiosSecure.get("http://localhost:8000/orders/all");
            return res.data;
        },
    });

    // ২. ফিল্টারিং লজিক (Search + Status Filter)
    const filteredOrders = orders.filter(order => {
        // স্ট্যাটাস ফিল্টার চেক
        const statusMatch = filterStatus === "all" || order.status === filterStatus;

        // সার্চ টেক্সট চেক (Buyer Name, Email বা Order ID দিয়ে)
                ...
                <div className="overflow-x-auto bg-white rounded-xl border border-slate-200 shadow-sm">
                    <table className="table table-sm">
                        ...
                        <tbody className="text-xs">
                            {/* ৪. এখানে filteredOrders ম্যাপ করা হয়েছে */}
                                            ...
                                            className={
                                                ...
                                                (order.status === "approved"
                                                    ...
                                                    : order.status === "rejected"
                                                        ...
                                                        : "bg-orange-100 text-orange-700 border-none") // Default Pending
                                            }
                                        >
