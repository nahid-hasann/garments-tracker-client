...
const OrderDetails = () => {
    ...
    const { data: order, isLoading, error, refe } = useQuery({
        queryKey: ["order", id],
        queryFn: async () => {
    ...
    const handleAddUpdate = async (e) => {
        e.preventDefault();
        if (!product) return;       // অথবা যদি order object use করো, তাহলে != product er jaygay order
        ...
        try {
            ...
            await axios.post(`http://localhost:8000/orders/${id}/production-updates`, {
                stage,
                note,
                updatedBy: "manager@factory.com", // আপাতত hard-code, পরে useAuth থেকে user.email দিতে পারো
            });
            ...
            // এখন টাটকা data আনতে হবে
            // তুমি যদি useEffect দিয়ে load করে থাকো:
            //   - simple ভাবে আবার load function call করতে পারো
            // তুমি যদি TanStack Query use করে থাকো:
            //   - useQuery থেকে refetch ব্যবহার করবে

            // উদাহরণ (যদি useEffect হয় and setProduct use করো):
            //   - addUpdate শেষে আবার order load করা:
            // await loadProduct();  // আলাদা করে ফাংশন বানালে
