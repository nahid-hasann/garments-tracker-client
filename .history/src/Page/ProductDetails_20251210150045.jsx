...
const ProductDetails = () => {
    ...
    // fallback: jodi field name different hoy (quantity, minimumOrder, paymentMode)
    const availableQuantity =
        availableQty ?? product.quantity ?? "Not set";
    const minOrderQty =
        minQty ?? product.minimumOrder ?? 0;
    const paymentMode =
        product.paymentMode ?? paymentOptions ?? "not-set";
    ...
    const handleOrderSubmit = async (e) => {
        ...
        const order = {
            ...
            address,
            notes: message,
            paymentMethod: paymentMode, // "Cash on Delivery" / "PayFirst"
            status: "pending",
            createdAt: new Date(),
        };
    ...
    const paymentInfoText =
        paymentMode === "cod"
            ? "Cash on Delivery"
            : paymentMode === "payfast" || paymentMode === "PayFirst"
                ? "PayFirst / Online Payment"
                        ...
                        <form onSubmit={handleOrderSubmit} className="space-y-3">
                            ...
                            <div className="space-y-1">
                                ...
                                <input
                                    type="text"
                                    value={
                                        orderPrice > 0 ? `$${orderPrice}` : "Will calculate automatically"
                                    }
                                    readOnly
                                    className="input input-bordered w-full text-sm bg-slate-50"
                                />
